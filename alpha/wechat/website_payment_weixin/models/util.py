...
BASE_DIR = os.path.dirname(os.path.dirname(os.path.normcase(__file__)))


def sign(data, key, sign_type=None):
    assert sign_type.upper() == 'MD5', "Sign type error"
    raw = [(k, str(data[k]) if isinstance(data[k], int) else data[k])
...
def verify_sign(data):
    _sign = data.pop('sign')
    wechat = request.env['payment.acquirer'].sudo().search(
        [('provider', '=', 'weixin')]
    )
    if _sign == sign(data, wechat.weixin_key, 'md5'):
        return True
    else:
        raise ValidationError(u'签名验证失败')


def json2xml(data):
    xml = ['<xml>']
    [xml.append('<{0}>{1}</{0}>'.format(k, v)) for k, v in data.items()]
    xml.append('</xml>')
...
def request_post(url, post, is_cert=False):
    __cert = os.path.join(BASE_DIR, 'chatter', 'p47mrPIG6N')
    __key = os.path.join(BASE_DIR, 'chatter', 'CxSLUm5WHp')
    headers = {'Content-Type': 'application/xml'}

    _logger.info('wechat pay API interface: %s', url)
    if is_cert:
        if not __cert or not __key:
            raise ValueError
        resp = requests.post(url, data=json2xml(post), headers=headers, cert=(__cert, __key))
    else:
        resp = requests.post(url, data=json2xml(post), headers=headers)

    _logger.info('wechat interface request data %s', pformat(json2xml(post)))
    resp.encoding = 'UTF-8'
    result = resp.text.encode("utf-8")
    return_xml = etree.fromstring(result)

    _logger.info('wechat interface response data %s', pformat(result))
    if return_xml.find('return_code').text == "SUCCESS" and return_xml.find('result_code').text == "SUCCESS":
        return return_xml
    else:
        try:
            return_code = return_xml.find('err_code').text
            return_msg = return_xml.find('err_code_des').text
        except Exception:
